...
func handler(ctx context.Context, request commons.InternalLMMReq) (commons.InternalLMMResp, error) {
	...
	for index, eachHost := range tmpNeo4jHosts {
		...
		if err != nil {
			apimodel.Anlogger.Errorf(lc, "messages.go : error making request to host [%s] : %v", eachHost, err)
			tmpNeo4jHosts = append(tmpNeo4jHosts[:index], tmpNeo4jHosts[index+1:]...)
			apimodel.Anlogger.Errorf(lc, "messages.go : result neo4j hosts %v", tmpNeo4jHosts)
			continue
		}
		...
		if httpResponse.StatusCode != 200 {
			apimodel.Anlogger.Errorf(lc, "messages.go : response from Neo4j NOT OK, requested host [%s], status code [%d] and status [%s]", eachHost, httpResponse.StatusCode, httpResponse.Status)
			tmpNeo4jHosts = append(tmpNeo4jHosts[:index], tmpNeo4jHosts[index+1:]...)
			apimodel.Anlogger.Errorf(lc, "messages.go : result neo4j hosts %v", tmpNeo4jHosts)
		} else {
