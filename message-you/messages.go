...
func handler(ctx context.Context, request commons.InternalLMMReq) (commons.InternalLMMResp, error) {
	...
	apimodel.Anlogger.Debugf(lc, "messages.go : start handle likes_you request %v", request)
	...
	if err != nil {
		apimodel.Anlogger.Errorf(lc, "messages.go : error marshaling request %v into json : %v", request, err)
		return commons.InternalLMMResp{}, fmt.Errorf("error marshaling request %v into json : %v", request, err)
	}

	apimodel.Anlogger.Debugf(lc, "messages.go : start iterate on hosts %v", tmpNeo4jHosts)
	for index, eachHost := range tmpNeo4jHosts {
		url := fmt.Sprintf("%s%s", eachHost, apimodel.MessageYouExtensionSuffix)
		apimodel.Anlogger.Debugf(lc, "messages.go : try this url [%s]", url)
		request, err := http.NewRequest("GET", url, bytes.NewReader(body))
		if err != nil {
			apimodel.Anlogger.Errorf(lc, "messages.go : error construction the request to host [%s] with body [%s] : %v", eachHost, string(body), err)
		...
		if err != nil {
			...
			apimodel.Anlogger.Errorf(lc, "messages.go : error making request to host [%s], result neo4j hosts %v : %v", eachHost, tmpNeo4jHosts, err)
			continue
		}
		...
		if err != nil {
			apimodel.Anlogger.Errorf(lc, "messages.go : error reading response body : %v", err)
			return commons.InternalLMMResp{}, fmt.Errorf("error reading response body : %v", err)
		}

		apimodel.Anlogger.Debugf(lc, "messages.go : response body %s", string(respBody))

		if httpResponse.StatusCode != 200 {
			tmpNeo4jHosts = append(tmpNeo4jHosts[:index], tmpNeo4jHosts[index+1:]...)
			apimodel.Anlogger.Errorf(lc, "messages.go : response from Neo4j NOT OK, requested host [%s], status code [%d] and status [%s], result neo4j hosts %v",
				eachHost, httpResponse.StatusCode, httpResponse.Status, tmpNeo4jHosts)
		} else {
			var response commons.InternalLMMResp
			err := json.Unmarshal(respBody, &response)
			if err != nil {
				apimodel.Anlogger.Errorf(lc, "messages.go : error unmarshaling response body [%s] in commons.InternalLMMResp : %v", string(respBody), err)
			...
			apimodel.Anlogger.Debugf(lc, "messages.go : successfully handle messages request in %v millis", commons.UnixTimeInMillis()-start)

			//round robin load balancer
			tmpNeo4jHosts = append(tmpNeo4jHosts[:index], tmpNeo4jHosts[index+1:]...)
			apimodel.Anlogger.Debugf(lc, "messages.go : result neo4j hosts %v", tmpNeo4jHosts)
			return response, nil
		}
	}
	apimodel.Anlogger.Errorf(lc, "messages.go : there is no alive hosts in %v", apimodel.Neo4jHosts)
